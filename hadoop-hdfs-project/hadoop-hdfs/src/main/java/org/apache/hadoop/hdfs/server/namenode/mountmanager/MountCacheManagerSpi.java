...
public interface MountCacheManagerSpi {

  /**
   * Factory for creating {@link MountCacheManagerSpi} objects.
   * @param <T>
   */
  abstract class Factory<T extends MountCacheManagerSpi> {
    ...
    public static Factory<?> getFactory(Configuration conf) {
      String className = conf.get(
          DFSConfigKeys.DFS_PROVIDED_CACHE_MANAGER_FACTORY,
          DFSConfigKeys.DFS_PROVIDED_CACHE_MANAGER_FACTORY_DEFAULT);
   ...
   */
  default void addBlock(BlockInfo blockInfo, DatanodeStorageInfo storageInfo) {
  }
   ...
   */
  default void removeBlocks(List<BlockInfo> blocks) {
  }
   ...
   * @param volInfo
   */
  default void addMount(Path mountPath, ProvidedVolumeInfo volInfo) {
  }

  /**
   * Remove mount path. Related to {@link MountCacheManagerSpi#addMount}
   * but called when the mount is removed.
   *
   * @param mountPath
   */
  default void removeMount(Path mountPath) {
  }
