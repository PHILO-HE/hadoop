...
/**
 * Maintains the replicas contained within a particular volume.
 * This class is not thread-safe and all "get"s to replicas should be done
 * through the {@link ReplicaMap}.
 */
class VolumeReplicaMap {

  protected AutoCloseableLock lock;

  // Map of block pool Id to a set of ReplicaInfo.
  private final Map<String, FoldedTreeSet<ReplicaInfo>> map;
  ...
  VolumeReplicaMap(AutoCloseableLock lock) {
    if (lock == null) {
   ...
   * @param other the VolumeReplicaMap which is to be merged with this map.
   *              {@code other} cannot be a subclass of VolumeReplicaMap.
   */
  void addAll(VolumeReplicaMap other) {
    if (!(other instanceof VolumeReplicaMap)) {
      throw new UnsupportedOperationException(
          "Only LeafReplicaMaps of the same type can be merged.");
    }
    try (AutoCloseableLock l = lock.acquire()) {
      try (AutoCloseableLock otherL = other.lock.acquire()) {
